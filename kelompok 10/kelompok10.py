...
class Kereta(KendaraanDarat):
    ...
    def info(self):
        return f"Kereta {self.nama} ({self.warna}) dengan rute {', '.join(self.rute)}\n" \
               f"Tahun Keluaran: {self.tahun_keluaran}\n Kecepatan: {self.kecepatan}km/h\n Bahan Bakar: {self.bahan_bakar}\n " \
               f"Jumlah Roda: {self.jumlah_roda}\n Kapasitas Penumpang: {self.kapasitas_penumpang}\n " \
               f"Jenis Layanan Kereta: {self.jenis_layanan_kereta}\n Jumlah Kursi: {self.jumlah_kursi}\n " \
...
class MobilBalap(Mobil):
    ...
    def info(self):
        return f"Mobil Balap {self.nama} ({self.warna})\n" \
               f"Tahun Keluaran: {self.tahun_keluaran}\n Kecepatan: {self.kecepatan}km/h\n Bahan Bakar: {self.bahan_bakar}\n " \
               f"Jumlah Roda: {self.jumlah_roda}\n Kapasitas Penumpang: {self.kapasitas_penumpang}\n " \
               f"Jenis Mobil: {self.jenis_mobil}\n Front Wing: {self.front_wing}\n Rear Wing: {self.rear_wing}\n"\
               f"Engine Status: {'On' if self.engine_status else 'Off'}"
class MobilCrossroad(Mobil):
    ...
    def info(self):
        return f"Mobil Crossroad {self.nama} ({self.warna})\n" \
               f"Tahun Keluaran: {self.tahun_keluaran}, Kecepatan: {self.kecepatan}km/h\n Bahan Bakar: {self.bahan_bakar}\n " \
               f"Jumlah Roda: {self.jumlah_roda}\n Kapasitas Penumpang: {self.kapasitas_penumpang}\n " \
               f"Jenis Mobil: {self.jenis_mobil}\n Sunroof Type: {self.sunroof_type}\n " \
...
def kereta():
    while True:
        ...
        if choice == "1":
            ...
            del_rute = input("Masukkan rute yang akan dihapus: ")
            kereta.kurangi_rute(del_rute)
            print(f"rute {del_rute} berhasil dihapus!")
        

        elif choice == "3":
           new_rute = input("Masukkan rute baru: ")
